...
{
	class ModeReplacement
	{
	public:

		bool operator<(const ModeReplacement& rhs) const;

		SpaceNavigatorAction::Action drivenBy;
		SpaceNavigatorAction::Action action;

		bool invertDrivingAxis;

		static ModeReplacement load(const boost::property_tree::ptree& tree);
	};
}
